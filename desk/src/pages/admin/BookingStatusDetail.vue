    ...
    <div class="sticky top-0 z-20 border-b border-gray-200 bg-white px-6 py-3 dark:border-gray-800 dark:bg-gray-900">
      <div class="flex items-center justify-between">
        <div class="flex items-center gap-3">
          <button @click="router.push('/admin/booking-statuses')" class="rounded-md p-1 text-gray-400 hover:bg-gray-100 hover:text-gray-600 dark:hover:bg-gray-700 dark:hover:text-gray-300">
      ...
      <div class="space-y-6 lg:col-span-2">
        ...
        <div class="rounded-lg border border-gray-200 bg-white shadow-sm dark:border-gray-700 dark:bg-gray-900">
          ...
          <div class="space-y-4 p-5">
            ...
            <div class="flex items-center gap-6">
              ...
              <label class="flex items-center gap-2 text-sm text-gray-700 dark:text-gray-300">
                <input v-model="form.is_final" type="checkbox" class="h-4 w-4 rounded border-gray-300 text-orange-600 focus:ring-orange-500 dark:border-gray-600 dark:bg-gray-700" />
                Final status
              </label>
            </div>
            <!-- Preview -->
        ...
        <div class="rounded-lg border border-red-200 bg-white shadow-sm dark:border-red-900/50 dark:bg-gray-900">
          ...
          <div class="flex items-center justify-between p-5">
            <div>
              <p class="text-sm font-medium text-gray-900 dark:text-white">Delete this booking status</p>
        ...
        <div class="rounded-lg border border-gray-200 bg-white shadow-sm dark:border-gray-700 dark:bg-gray-900">
          ...
          <div class="divide-y divide-gray-100 dark:divide-gray-700">
            ...
            <div class="flex justify-between px-5 py-2.5">
              ...
              <span class="text-xs font-medium" :class="doc.is_final ? 'text-orange-600 dark:text-orange-400' : 'text-gray-500'">{{ doc.is_final ? 'Yes' : 'No' }}</span>
            </div>
            <div class="flex justify-between px-5 py-2.5">
...
function goToPrevious() {
  ...
  router.push(`/admin/booking-statuses/${allStatusIds.value[prevIdx]}`)
}
function goToNext() {
  if (!hasNext.value) return
  const nextIdx = (currentIndex.value + 1) % allStatusIds.value.length
  router.push(`/admin/booking-statuses/${allStatusIds.value[nextIdx]}`)
}
...
// Form state
const form = reactive({ status: '', color: '#3b82f6', is_active: true, is_final: false })
...
async function loadActivities() {
  ...
  try {
    const versions = await call('frappe.client.get_list', {
      doctype: 'Version',
      filters: { ref_doctype: 'MM Booking Status', docname: currentId.value },
...
async function executeDelete() {
  deleting.value = true
  try {
    await call('frappe.client.delete', { doctype: 'MM Booking Status', name: currentId.value })
    toast({ title: 'Status color deleted', icon: 'check' })
    router.push('/admin/booking-statuses')
  } catch (e) {
