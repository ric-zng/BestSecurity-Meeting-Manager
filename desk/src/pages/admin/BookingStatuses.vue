<template>
  <div class="relative flex h-full flex-col bg-gray-50 dark:bg-gray-950">
    ...
    <div class="flex items-center justify-between border-b border-gray-200 bg-white px-6 py-4 dark:border-gray-800 dark:bg-gray-900">
      <div>
        <h1 class="text-lg font-semibold text-gray-900 dark:text-white">Booking Statuses</h1>
    ...
    <div class="flex-1 overflow-auto">
      ...
      <EmptyState
        ...
        icon="droplet"
        title="No booking statuss found"
        :description="hasActiveFilters ? 'Try adjusting your filters' : 'Create your first booking status to get started'"
      >
        <template #action>
          <button
            v-if="!hasActiveFilters"
        ...
        <table class="w-full text-sm">
          <thead>
            <tr class="border-b border-gray-200 dark:border-gray-700">
              ...
              <th class="bg-gray-50 px-4 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-500 dark:bg-gray-900/50 dark:text-gray-400">Color</th>
              <th class="bg-gray-50 px-4 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-500 dark:bg-gray-900/50 dark:text-gray-400">Status Name</th>
              <th class="bg-gray-50 px-4 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-500 dark:bg-gray-900/50 dark:text-gray-400">Hex Code</th>
              <th class="bg-gray-50 px-4 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-500 dark:bg-gray-900/50 dark:text-gray-400">Preview</th>
              <th class="bg-gray-50 px-4 py-3 text-center text-xs font-medium uppercase tracking-wider text-gray-500 dark:bg-gray-900/50 dark:text-gray-400">Bookings</th>
              <th class="bg-gray-50 px-4 py-3 text-center text-xs font-medium uppercase tracking-wider text-gray-500 dark:bg-gray-900/50 dark:text-gray-400">Final</th>
          ...
          <tbody class="divide-y divide-gray-100 dark:divide-gray-800">
            ...
            >
              ...
              <td class="px-4 py-3 text-center">
                <FeatherIcon v-if="item.is_final" name="check" class="mx-auto h-4 w-4 text-orange-500" />
    ...
    <!-- Pagination -->
    <div v-if="totalCount > pageSize" class="border-t border-gray-200 bg-white px-6 py-3 dark:border-gray-800 dark:bg-gray-900">
      <div class="flex items-center justify-between text-sm">
        ...
        <div class="flex items-center gap-1">
          <button
            ...
            class="rounded-md border border-gray-300 bg-white px-2.5 py-1 text-gray-600 hover:bg-gray-50 disabled:opacity-40 dark:border-gray-600 dark:bg-gray-900 dark:text-gray-400 dark:hover:bg-gray-700"
          >
    ...
    <teleport to="body">
      ...
      >
        <div class="w-full max-w-md rounded-lg border border-gray-200 bg-white p-6 shadow-xl dark:border-gray-700 dark:bg-gray-900">
          ...
          <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Create a new booking status for bookings</p>
          ...
          <div class="mt-4 space-y-4">
            ...
            <label class="flex items-center gap-2 text-sm text-gray-700 dark:text-gray-300">
              <input v-model="newStatus.is_final" type="checkbox" class="h-4 w-4 rounded border-gray-300 text-blue-600 focus:ring-blue-500 dark:border-gray-600 dark:bg-gray-700" />
              Final status (bookings with this status cannot be modified)
            </label>
          </div>
...
const totalLabel = computed(() => {
  ...
  return `${totalCount.value} booking status${totalCount.value !== 1 ? 's' : ''}${hasActiveFilters.value ? ' (filtered)' : ''}`
})
  ...
  try {
    ...
    const [list, count] = await Promise.all([
      call('frappe.client.get_list', {
        ...
        fields: ['name', 'status', 'color', 'is_active', 'is_final'],
        filters,
        order_by: 'status asc',
...
async function executeConfirmedAction() {
  ...
  try {
    if (action === 'toggle_active') {
      ...
      for (const name of names) {
        try {
          await call('frappe.client.delete', { doctype: 'MM Booking Status', name })
          succeeded++
        } catch { failed++ }
      }
      // Optimistic UI
...
const addError = ref('')
const newStatus = ref({ name: '', color: '#3b82f6', is_final: false })
  ...
  try {
    const doc = await call('frappe.client.insert', {
      doc: {
        ...
        is_active: 1,
        is_final: newStatus.value.is_final ? 1 : 0,
      },
    })
    showAddModal.value = false
    newStatus.value = { name: '', color: '#3b82f6', is_final: false }
    toast({ title: `Added "${doc.status}"`, icon: 'check' })
    router.push(`/admin/booking-statuses/${doc.name}`)
  } catch (err) {
...
function goToDetail(name) {
  router.push(`/admin/booking-statuses/${name}`)
}
